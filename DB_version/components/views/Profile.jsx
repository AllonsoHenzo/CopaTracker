...
export default function ProfilePage() {
  ...
  const { watchedSet, state, toggleWatched, updateProfile, setTheme, setLang, setFavoriteTeam, resetAll, matches, user, logout } = useStore();
  const t = useT();
  ...
  return (
    <div className="page-in">
      ...
      <div className="sec-head">
        ...
        <h2 className="sec-title">Minha <span className="it">Conta</span></h2>
      </div>
      <section style={{ padding: '0 40px 40px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
          <div>
            <div className="mono" style={{ marginBottom: 4, opacity: 0.5 }}>EMAIL DA CONTA</div>
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 22, letterSpacing: '-0.02em' }}>{user?.email}</div>
          </div>
          <button
            className="btn"
            onClick={async () => { if (confirm('Sair da conta?')) await logout(); }}
            style={{ color: '#ff4d4d', borderColor: '#ff4d4d' }}
          >
            Sair da conta
          </button>
        </div>
        <div style={{ paddingTop: 16, borderTop: '1px solid var(--line-faint)' }}>
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 24, letterSpacing: '-0.03em', marginBottom: 4 }}>{t.profile.reset_title}</div>
          <div className="mono" style={{ marginBottom: 16, opacity: 0.5 }}>{t.profile.reset_hint}</div>
          <button className="btn btn-sm" onClick={() => { if (confirm(t.profile.reset_confirm)) resetAll(); }}>
            {t.profile.reset_btn}
          </button>
        </div>
      </section>
